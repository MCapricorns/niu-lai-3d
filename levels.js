...
/**
 * v4.0.0 "I Wanna Be The Ox" · 单一连续跳刺大地图
 *
 * 直接照抄 IWBTG 系列的流程骨架:
 * - 一张 475 格长图从头走到尾,没有选关,没有小关分段;
 * - 十一枚发光存档点(save)把全程切成十六大考段,碰到即存,死亡从存档点满状态重来;
 * - 即死陷阱密度拉满,一段只考一件事:
 *   刺缝入门 → 碎板假桥 → 炮火走廊(平射弹) → 岩浆跳岛 → 弹簧陷阱(满弹上天堂) →
 *   熔岩塔阶+蹬墙井 → 双子摆渡 → 碎板炮火复合 → 窄板刺海 → 豹之领土 →
 *   GPT 老板守关 → 炮台阶梯 → 顶刺疾走 → 岩浆碎板桥 → 窄板之巅 → 再守关 → 机房决战;
 * - 段与段之间留 2-6 格绝对干净的"呼吸平台",失败原因永远可读;
 * - 终点是 Anthropic Dario 机房决战,打倒即通关。
 *
 * 物理标定(T=40px,默认全速):小跳顶点≈4.6格 / 二段跳≈4.1格 /
 * 跑跳水平≈8格 / 弹簧(点弹≈4.7格,满弹≈9.3格) / 蹬墙跳≈4.2格。
 * 关卡布局注释中的高度/距离均按这套数字标定;改手感请先核对全图。
 *
 * Tile legend:
...
window.createNiuLaiLevels = function createNiuLaiLevels(TAU) {
  ...
  defLevel(475, "I Wanna Be The Ox", 5, function (g) {
    g.startX = 3;
    ...
    /* —— 321..349 S12 炮台阶梯:台阶一层层上,高台炮火扫爬升线 —— */
    g.mesa(319, 320, 10); /* 高台炮塔 */
    g.cannon(319, 9.25); /* 弹道扫过台阶2的站面:读弹,别在二层久留 */
    g.mesa(325, 326, 11); /* 台阶1:弹道上方,安全 */
    g.spikeBelt(327, 329);
    g.mesa(330, 331, 10); /* 台阶2:弹道高度,借过别站 */
    g.spikeBelt(332, 334);
    g.mesa(335, 336, 9); /* 台阶3:弹线之上,俯瞰全段 */
    g.coinArc(330, 8);
    g.mesa(342, 343, 10); /* 下撤缓冲 */
    g.bigc(340, 7);
    g.save(347);

    /* —— 350..376 S13 顶刺疾走:读檐走位,该走就走,该跳才跳 —— */
    g.ceil(353, 357, 10); /* 低檐:只许走 */
    g.coinRow(353, 11, 5);
    g.spikeBelt(360, 361); /* 无檐段:两格贴地刺,小跳 */
    g.ceil(364, 368, 10); /* 再低檐 */
    g.spikeBelt(371, 372);
    g.coinArc(370, 8);
    g.save(375);

    /* —— 377..408 S14 岩浆碎板桥:塌桥上补跳,对岸炮火不断 —— */
    g.pit(381, 402);
    g.lava(381, 22); /* 381..402 岩浆河 */
    g.cr(383, 10, 11); /* 上层碎板桥 383..392 */
    g.cr(395, 8, 11); /* 空档 393..394 中途补跳,再落到 395..402 */
    g.coinRow(385, 9, 4);
    g.bigc(393, 8);
    g.ground(403, 410);
    g.mesa(404, 405, 11); /* 对岸炮塔 */
    g.cannon(404, 10.25); /* 高位平射:弹道扫下桥的你 */
    g.save(408);

    /* —— 409..434 S15 窄板之巅:像素落点·进阶版 —— */
    g.spikePit(411, 428);
    g.plat(413, 10, 1);
    g.plat(417, 9, 1);
    g.plat(421, 10, 1);
    g.plat(425, 9, 1); /* 4 块窄板,4 格间距高低交替 */
    g.bigc(423, 6);
    g.ground(429, 446);
    g.save(432);

    /* —— 435..446 S16 再守关:第二位 GPT 老板 —— */
    g.mesa(438, 439, 10); /* 掩体 */
    g.miniboss(443);

    /* —— 447..474 S17 机房决战:Anthropic Dario —— */
