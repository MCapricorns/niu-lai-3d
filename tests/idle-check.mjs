...
for (let i = 0; i < 100; i++) {
  try {
    const res = await fetch(`http://127.0.0.1:${debugPort}/json/list`, { signal: AbortSignal.timeout(1000) });
...
async function evaluate(expression) {
  ...
  if (out.result?.exceptionDetails)
    console.log("EXC:", JSON.stringify(out.result.exceptionDetails.exception?.description || "").slice(0, 300));
...
const expr = `(function(){
  ...
  for(var f=0;f<60*8;f++){
    ...
    if(f%20===0||GS.state!=="play")
      log.push("A f"+f+" st:"+GS.state+" x"+Math.round(PL.x)+" y"+Math.round(PL.y)+" dead:"+(!!PL.dead)+" inv:"+Math.round(PL.inv*10)/10);
